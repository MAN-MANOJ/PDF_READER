...
# Global variable to track the latest JSON path
latest_json_path = ""
...
def search_topic(text_data, keyword):
    ...
    for page_num, text in text_data.items():
        if keyword.lower() in text.lower():
            results.append(f"Page {page_num}: {text[:200]}...")  # Show only first 200 chars
...
def process_pdf():
    ...
    latest_json_path = os.path.join(output_folder, "extracted_results.json")
    with open(latest_json_path, "w", encoding="utf-8") as json_file:
